      ...
      if (user_id !== null) {
        ...
        if (store_offer_id !== null) {
          const { data, error } = await supabase
            ...
            .insert([
              {
                store_offer_id: selectedItemData.store_offer_id,
                user_id: user_id,
                time_stamp: new Date(),
              }
            ])
          ...
          <div className="w-4/6 max-w-screen-xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {gameStoreData.map(data => (
              <a
                ...
                key={data.id} // Make sure this key is unique
      ...
      {errorModal && ( 
        <div className='fixed top-0 left-0 p-5 w-full h-screen flex justify-center items-center bg-gray-600 bg-opacity-50 z-40'>
          <div className='flex flex-col items-center gap-5 p-5 bg-white shadow-2xl rounded-md'>
            ...
            <button className='p-1 hover-bg-red-500 rounded-full duration-150' type='button' onClick={() => setErrorModal(false)}>
              <p className='text-4xl'> <AiOutlineCloseCircle /> </p>
            </button>
          </div>
        </div>
      )}
      {successModal && (
        <div className='fixed top-0 left-0 p-5 w-full h-screen flex justify-center items-center bg-gray-600 bg-opacity-50 z-40'>
          <div className='flex flex-col items-center gap-5 p-5 bg-white shadow-2xl rounded-md'>
            ...
            <button className='p-1 hover-bg-red-500 rounded-full duration-150' type='button' onClick={() => setSuccessModal(false)}>
              <p className='text-4xl'> <AiOutlineCloseCircle /> </p>
            </button>
          </div>
        </div>
